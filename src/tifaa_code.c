...
void
allocate_stings(struct Config *conf)
/***********************************************
 ** Allocate space to all the strings in the  **
 ** configuration structure.                  **
 ***********************************************/
{
    /* Allocate the space: */
    conf->cat_add   = malloc(LINE_BUFFER*sizeof(char));
    conf->surv_add  = malloc(LINE_BUFFER*sizeof(char));
    conf->out_add   = malloc(LINE_BUFFER*sizeof(char));
    conf->out_ext   = malloc(LINE_BUFFER*sizeof(char));
    conf->img_pfx   = malloc(LINE_BUFFER*sizeof(char));
    conf->info_name = malloc(LINE_BUFFER*sizeof(char));
    
    /* Check if space could be allocated: */
    if (conf->cat_add==NULL || conf->surv_add==NULL 
        || conf->out_add==NULL || conf->img_pfx==NULL
        || conf->info_name==NULL || conf->out_ext==NULL)
    {
        printf("\n   ### Error: Space could not be");
        printf("allocated\n       to at least one of the strings");
...
void
reallocate_strings(struct Config *conf)
/***********************************************
 ** Initially the space allocated to the      **
 ** strings in the structure was too much.    **
 ** Here the space for each string is         **
 ** corrected to only contain the final string**
 ***********************************************/
{
    /* re-allocate the space: */
    conf->cat_add   = realloc(conf->cat_add, 
            (strlen(conf->cat_add)+1)  *sizeof(char));
    conf->surv_add  = realloc(conf->surv_add, 
            (strlen(conf->surv_add)+1) *sizeof(char));
    conf->out_add   = realloc(conf->out_add, 
            (strlen(conf->out_add)+1)  *sizeof(char));
    conf->out_ext = realloc(conf->out_ext, 
            (strlen(conf->out_ext)+1)*sizeof(char)); 
    conf->img_pfx   = realloc(conf->img_pfx, 
            (strlen(conf->img_pfx)+1)  *sizeof(char));
    conf->info_name = realloc(conf->info_name, 
            (strlen(conf->info_name)+1)*sizeof(char)); 

    /* Check if space was reallocated: */
    if (conf->cat_add==NULL || conf->surv_add==NULL 
        || conf->out_add==NULL || conf->img_pfx==NULL
        || conf->info_name==NULL || conf->out_ext==NULL)
    {
...
void
no_config_exit(char *config_name)
/***********************************************
 ** If a configuration setting was not defined**
 ** Report it and exit.                       **
 ***********************************************/
{
    printf("\n   ### Error: No value was set for: ");
    printf("%s \n\n", config_name);
    exit(EXIT_FAILURE); 
}

void 
tifaa_read_config(char *conf_name, struct Config *conf)
/*********************************************** 
 ** This function will read the configuration ** 
 ** file name as given by the first input     **
 ** parameter and fill up the configuration   **
 ** structure given as the second array.      **
 ***********************************************/
{
    /* Declaratins: */
    char line[LINE_BUFFER];
    char *name, *valuestr, *tempstr="";
    char **ExtraString=&tempstr;
    FILE *fp=fopen(conf_name, "r");
    if (fp==NULL)
    {
        printf("\n\n  ## Error: %s could not be opened, aborted.\n", conf_name);
        exit(EXIT_FAILURE);
    }

    /* Allocate space to all the srings in the Config structure */
    allocate_stings(conf);

    /* Initialize the conf structure: */
    strcpy(conf->cat_add, "");
    strcpy(conf->surv_add, "");
    strcpy(conf->out_add, "");
    strcpy(conf->out_ext, "");
    strcpy(conf->img_pfx, "");
    strcpy(conf->info_name, "");
    conf->id_col =-1;
    conf->ra_col =-1;
    conf->dec_col=-1;
    conf->res    =-1;
    ...
    /* Go over the configuration file line by line 
    and fill it in: */
    while(!feof(fp))
    {
        /* Read one line and separate the name from
        the value of the configuration line: */
        fgets(line, sizeof(line), fp);
        if (line[0]=='#') continue;

        /* Check if the length of the line is shorter 
        than the space we have allocated for it */
        if (strlen(line)>LINE_BUFFER)
        {
            printf("  ## Error: LINE_BUFFER is too short.\n\n");
            exit(EXIT_FAILURE);
        }

        /* Read the lines: If there isn't a value after 
        the configuration setting, then exit: */
        if( (name=strtok(line," \n=,:"))==NULL ) continue;
        if( (valuestr=strtok(NULL," \n,=:"))==NULL )
        {
            printf("\n ## Error: The config parameter ");
            printf("%s has no value\n\n", name);
            exit(EXIT_FAILURE);
        }
        /*strcat(valuestr, "\0");*/

        /* Depending on the config name, set the value: */
        if (strcmp(name,"CATALOG_ADDRESS")==0) 
            strcpy(conf->cat_add, valuestr);
        else if (strcmp(name,"SURVEY_ADDRESS")==0) 
            strcpy(conf->surv_add, valuestr);
        else if (strcmp(name,"OUTPUT_ADDRESS")==0)  
            strcpy(conf->out_add, valuestr);
        else if (strcmp(name,"OUTPUT_EXTEN")==0)  
            strcpy(conf->out_ext, valuestr);
        else if (strcmp(name,"IMG_PREFIX")==0)  
            strcpy(conf->img_pfx, valuestr);
        else if (strcmp(name,"IMG_INFO")==0)  
            strcpy(conf->info_name, valuestr);
        else if (strcmp(name,"ID_COLUMN")==0)
            conf->id_col =strtol(valuestr, (char **)NULL, 10);
        else if (strcmp(name,"RA_COLUMN")==0)
            conf->ra_col =strtol(valuestr, (char **)NULL, 10);
        else if (strcmp(name,"DEC_COLUMN")==0)
            conf->dec_col=strtol(valuestr, (char **)NULL, 10);
        else if (strcmp(name,"CHECK_SIZE")==0)
            conf->chk_siz=strtol(valuestr, (char **)NULL, 10);
        else if (strcmp(name,"RESOLUTION")==0)
        {
            conf->res=strtod(valuestr, ExtraString); 
            if (strlen(*ExtraString)>0) 
            {
                printf("   ### Value of RESOLUTION is not readable!");
                exit(EXIT_FAILURE);
            }
        }
        else if (strcmp(name,"PS_SIZE")==0)
        {
            conf->ps_size=strtod(valuestr, ExtraString); 
            if (strlen(*ExtraString)>0) 
            {
                printf("   ### Value of PS_SIZE is not readable!");
                exit(EXIT_FAILURE);
            }
        }
        else 
    ...
    /* Check if all the parameters were read: */
    if ( (long)strlen(conf->cat_add)==0 )
        no_config_exit("CATALOG_ADDRESS");
    if ( (long)strlen(conf->surv_add)==0 )
        no_config_exit("SURVEY_ADDRESS");
    if ( (long)strlen(conf->out_add)==0 )
        no_config_exit("OUTPUT_ADDRESS");
    if ( (long)strlen(conf->out_ext)==0 )
        no_config_exit("OUTPUT_EXTEN");
    if ( (long)strlen(conf->img_pfx)==0 )
        no_config_exit("IMG_PREFIX");
    if ( (long)strlen(conf->info_name)==0 )
        no_config_exit("IMG_INFO");
    if ( conf->id_col==-1 )
        no_config_exit("ID_COLUMN");
    if ( conf->ra_col==-1 )
        no_config_exit("RA_COLUMN");
    if ( conf->dec_col==-1 )
        no_config_exit("DEC_COLUMN"); 
    if ( conf->chk_siz==-1 )
        no_config_exit("CHECK_SIZE"); 
    if ( conf->res==-1 )
        no_config_exit("RESOLUTION"); 
    if ( conf->ps_size==-1 )
        no_config_exit("PS_SIZE"); 

    /* The directory addresses have to have a '/' as 
    the last character, if they don't add one: */ 
    if (conf->surv_add[strlen(conf->surv_add)-1]!='/')
        strcat(conf->surv_add, "/");
    if (conf->out_add[strlen(conf->out_add)-1]!='/')
        strcat(conf->out_add, "/");

    /* Correct the space allocated for the strings to
    only the space needed. */
    reallocate_strings(conf);
}

void
free_config(struct Config *conf)
/*********************************************** 
 ** After the job is done, this function will **
 ** free the space that was allocated to the  **
 ** strings in the configuration structure    **
 ***********************************************/
{
    free(conf->cat_add); free(conf->surv_add);
    free(conf->out_add); free(conf->out_ext);
    free(conf->img_pfx); free(conf->info_name);
}
...
/***********************************************
 ** This function looks into the directory    **
 ** and outputs the names of all the desired  **
 ** files.                                    **
 ***********************************************/
{
    /* Declarations: 
...
/***********************************************
 ** This function will open a FITS file, read **
 ** the header and output a prepared wcsprm   **
 **                                           **
...
void
get_imginfo(char *fits_name, double **imginfo, 
        unsigned long zero_pos, const double res)
/***********************************************
 ** WCSLIB requires a string made up of all   **
 ** the headers in a fits file, this function **
 ** will get that string using CFITSIO.       **
 ***********************************************/
{
    /* For prepare_fitswcs: */
    ...
    /* For converting coordinates, note that here 
    we just want to convert one point, so ncoord=1,
    if you want more than one point, change this to
    that number.*/
    int stat[NWCSFIX];
    ...
    a conversion for one point, for more than one point, just change ncoord. */
    w_status = wcsp2s(wcs, ncoord, nelem, pixcrd, imgcrd, &phi, &theta, world, stat);
    if (w_status!=0)
    {
    ...
    /* Save the results into the table. About the last two
    values: It is important to know how far (in degrees)
    the sides of the images are to their center. So here I
    find the angular distance of half the width of the image
    in each axis (in degrees). */
...
fits_info(struct Config *conf, glob_t *fits_names, 
        double **imginfo)
/***********************************************
 ** This function will read the header of all **
 ** FITS files in the folder and save the     **
 ** necessary information (image name, central**
 ** position RA, central position Dec, NAXIS1 **
 ** and NAXIS2) into an array.                **
 ***********************************************/
{
    /* Declarations: */
    unsigned long i;
...
/**********************************************
 ** This function finds the unique elements  **
 ** in an array, the pointer "felem" points  **
 ** to the first desired element in the array**
 ** the number of elements you want to order **
 ** is given by num_elem and the step size   **
 ** tells how far the next element is in the **
 ** original array (in case it is 1D this is **
 ** 1, if it is 2D, step_size=number of      **
 ** columns.                                 **
 **********************************************/
{
    /* Declaratins: */
...
void 
obj_fits(struct ArrayInfo *intable, 
        struct Config *conf, glob_t *fits_names, 
        double **imginfo, int **which_images)
/***********************************************
 ** This function will look at the 4 corners  **
 ** of a square that has been identified by   **
 ** the user around the object's RA and Dec.  **
 ** It will then find the images that contain **
...
/***********************************************
 ** The FITS standard assumes that pixels are **
 ** counted from their center and in integer  **
 ** values. So the bottom left corner of a    **
 ** FITS image (in a continues space) has     **
 ** coordinates: (0.5,0.5). In converting     **
 ** from WCS to pixel, wcslib gives continues **
 ** values, this function converts that into  **
...
/***********************************************
 ** Given the coordiantes of the object, this **
 ** function finds which pixels of the image  **
 ** correspond to which pixels in the cropped **
 ** image using the *pixel and *pixel_c arrays**
 **                                           **
 ** NOTE: I am going with the cfitsio standard**
 **   here: the desired region boundries are  **
 **   included.
 ***********************************************/
{
    /* Declarations: */
...
/*************************************************
 ** Check the central pixels of the finalized   **
 ** image to see if they aren't zero, if they   **
 ** are, set the remove variable on (=1). Note  **
...
/***********************************************
 ** This function will report the result for  **
 ** each image and set the remove_flag        **
 ***********************************************/
{
    if (zero_flag==1)
    {
...
/***********************************************
 ** This function will pull out those objects **
 ** that only need one image, to be most      **
 ** efficient, it will first open each FITS   **
 ** file and then pull out all the objects    **
 ** that are in it.                           **
 ***********************************************/
{
    /* Declarations: */
    ...
    for(pt=unique;pt<unique+intable->s0;pt++)
    {
        ...
        prepare_fitswcs(fits_names->gl_pathv[*pt], &read_fptr, &fr_status, 
                &wc_status, &nwcs, &wcs);
        fits_read_key(read_fptr, TLONG, "NAXIS1", &inaxes[0], NULL, &fr_status);
        fits_read_key(read_fptr, TLONG, "NAXIS2", &inaxes[1], NULL, &fr_status);
        ...
        for(i=0;i<intable->s0;i++)
        {
            ...
            fits_read_subset_flt(read_fptr, group, naxis, inaxes, fpixel_i, lpixel_i, 
                    inc, nulval, cropped, &anynul, &fr_status);
...
needs_more_than_one(struct ArrayInfo *intable, struct Config *conf, 
        glob_t *fits_names, int **which_images, double **final_report)
/***********************************************
 ** This function will crop objects that need **
 ** more than one tile. For each object, the  **
 ** required section of each tile is brough   **
 ** out and placed into the output array.     **
 ***********************************************/
{
    /* Declarations: */
    long naxis=2, crop_side, onaxes[2], inaxes[2], nelements, inc[2]={1,1};
    ...
    for(i=0;i<intable->s0;i++)
    {
        ...
        while((*which_images)[i*WI_COLS+imagesdone]!=-1 && imagesdone<WI_COLS)
        {
            ...
            prepare_fitswcs(fits_names->gl_pathv[(*which_images) 
                    [i*WI_COLS+imagesdone]], &read_fptr, &fr_status, 
                    &wc_status, &nwcs, &wcs);
            fits_read_key(read_fptr, TLONG, "NAXIS1", &inaxes[0], NULL, &fr_status);
            fits_read_key(read_fptr, TLONG, "NAXIS2", &inaxes[1], NULL, &fr_status);
            ...
            fits_read_subset_flt(read_fptr, group, naxis, inaxes, fpixel_i, lpixel_i, 
                    inc, nulval, temparray, &anynul, &fr_status);
...
stichcrop(struct ArrayInfo *intable, struct Config *conf,
        glob_t *fits_names, int **which_images, double **final_report)
/***********************************************
 ** Knowing exactly which images are required **
 ** for each object, this function will crop  **
 ** the required region out for each image    **
 ***********************************************/
{
    /* Declarations: */
    int filecheck=0;
...
void 
print_report(struct ArrayInfo *intable, 
        struct Config *conf, double **final_report, 
        time_t *t_start, time_t *t_1, time_t *t_end, 
        glob_t *fits_names)
/***********************************************
 ** Print the report                          **
 ***********************************************/
...
tifaa_crop_from_survey(struct ArrayInfo *intable, 
        struct Config *conf)
/***********************************************
 ** This function will first get all the      **
 ** required information, then it will take   **
 ** the desired sections of each image and    **
